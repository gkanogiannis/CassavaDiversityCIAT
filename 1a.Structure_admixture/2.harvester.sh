...
do
	K=$((10#$K+0))
	inds=$(grep -m 1 'Size of G' ${input}/K`printf %02d $K`/*.admixture.log |cut -d'x' -f1|cut -d' ' -f4)
	...
	do
		paste \
		<(for ((i=1;i<=$inds;i++));do echo "$i:";done) ${input}/K`printf %02d $K`/*.run_`printf %02d $run`.Q \
